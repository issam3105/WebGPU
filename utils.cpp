...
Texture Utils::loadTexture(unsigned char* pixelData, int& width, int& height, int& channels, TextureView* pTextureView) {
...
Texture Utils::loadImageFromPath(const std::string& path, TextureView* pTextureView) {
	int width, height, channels;
	unsigned char* data = stbi_load(path.c_str(), &width, &height, &channels, 4);
	if (!data) {
		std::cerr << "Failed to load image from path: " << path << std::endl;
	}
	return loadTexture(data, width, height, channels, pTextureView);
}
